...
def get_args():
    ...
    # basic
    parser.add_argument('--src_city', type=str, default='sh',
                        help='The city be as a source data for transfer')
    parser.add_argument('--trg_city', type=str, default='hz',
                        help='The city needed to be trasferred')
...
if __name__ == '__main__':
    ...
    config['basic']['device'] = torch.device(
    ...
    logger.info(
        f"src_city: {config['basic']['src_city']}\ttrg_city: {config['basic']['trg_city']}\tdevice: {config['basic']['device']}\t")
    # cluster_process
    if not config['cluster']['results']['path']:
        cluster_num, final_res, domain_label, pth = cluster_main(
            num_cluster, config)
